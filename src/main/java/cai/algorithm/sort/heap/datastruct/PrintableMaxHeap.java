...
public class PrintableMaxHeap<T extends Comparable<T>>  extends MaxHeap  {
    private final Logger logger = LoggerFactory.getLogger(PrintableMaxHeap.class);
...
//            if (data[i].compareTo(100) < 0 || data[i].compareTo(100) > 0) {
//                logger.error("数字不在[0,100]内不输出！！！");
//            }
    ...
    private String putNumberInLine(Integer num, String line, int indexCurLevel, int curTreeWidth, boolean isLeft) {
        ...
        if (offset + 1 > line.length()) {
            logger.error("offset+1 >line.length() : [offset:" + offset + 1 + ",line.length():" + line.length() + "]");
        }
        if (num >= 10)
